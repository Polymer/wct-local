...
function detect(done) {
  launchpad.local(function(error, launcher) {
    ...
    launcher.browsers(function(error, browsers) {
      ...
      var results = {};
      async.forEachOf(browsers, function(browser, index, cb) {
        var fn = LAUNCHPAD_TO_SELENIUM[browser.name];
        if (fn) {
          fn(browser, function(capabilities) {
            results[browser.name] = capabilities;
            cb();
          });
        }
      }, function() {
 ...
 * @return {!Object} A selenium capabilities object.
 */
function firefox(browser, cb) {
  var fp = new FirefoxProfile();
  fp.setPreference('marionette.logging', 'TRACE');
 ...
 * @return {!Object} A selenium capabilities object.
 */
function internetExplorer(browser, cb) {
  cb({
